...
def installed() {
    ...
    sendEvent([name:'heatingSetpoint', value:'60', displayed:false])
    sendEvent([name:'coolingSetpoint', value:'76', displayed:false])
...
def setHeatingSetpoint(temp) {
    ...
    return writeTstatValue('t_heat', t)
}
...
def setCoolingSetpoint(temp) {
    ...
    return writeTstatValue('t_cool', t)
}
...
// polling.poll 
def poll() {
    log.debug "poll(${state.hostAddress})"
    return refresh()
}
...
private apiGet(String path) {
	...
	log.debug("apiGet(${path}) : ${httpRequest}")
...
private updateDNI() {
	...
	log.debug("deviceDNI:${device.deviceNetworkId}, stateDNI:${state.dni}")
    return true
}
